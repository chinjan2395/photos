...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if (!$model) {
            $model = 0;
            $this->redirect('index.php/site/create');
        }
        $this->render(
    ...
    //        Get Heading row for table column name
        for ($j = 2; $j < $_POST['highestRow']; $j++) {
            for ($i = 1; $i <= $_POST['highestColumm']; $i++) {

                $array[] = $_POST['head_row_0' . 'col_' . $i];
    //        Check if header contains space or special characters
                if (strpos($_POST['head_row_0' . 'col_' . $i]," ") || strpos($_POST['head_row_0' . 'col_' . $i],"/")) {
                    $array[$i-1]=str_replace(" ","_",$_POST['head_row_0' . 'col_' . $i]);
                    $array[$i-1]=str_replace("/","_",$_POST['head_row_0' . 'col_' . $i]);
                }
            }
            break;
    ...
    //        Create table query
        $sqlQuery = "CREATE TABLE IF NOT EXISTS ".$_POST['t_name']." (" . implode(" VARCHAR(200),", $array) . " VARCHAR(200))";
    ...
    //        Get excel data
        for ($j = 2; $j < $_POST['highestRow']; $j++) {
            for ($i = 1; $i <= $_POST['highestColumm']; $i++) {
                ...
                if (strpos($_POST['row_' . ($j - 1) . 'col_' . $i],"'")) {
                    $_POST['row_' . ($j - 1) . 'col_' . $i] = str_replace("'"," ",$_POST['row_' . ($j - 1) . 'col_' . $i]);
                }
                $arr[$j - 2][$i] = $_POST['row_' . ($j - 1) . 'col_' . $i];
            }
    //        Insert query
            $sql_insert = "INSERT INTO ".$_POST['t_name']." (" . implode(",", $array) . ") VALUES ('" . implode("','", $arr[$j - 2]) . "')";
            $sqlCommand_insert = Yii::app()->db->createCommand($sql_insert);
